...
## Версия: июнь 2026 (обновление 8.31.0) — Редизайн v2 «sage shadcn», desktop-only

> Крупнейший UI-релиз: полный редизайн под язык sprint-planner-cloud (sage-green
> shadcn, Inter, токены), desktop-only, авто-сортировка списка по Priority Score,
> диалог параметров печати и валидация параметров спринта. 40 волн (W1–W40),
> каждая с зелёными гейтами.

### Главное

- **Редизайн v2**: светлая sage-тема (смягчённая, W29) — дефолт; тёмная — глубокий
  navy со «стеклянными» панелями дашборда (W19/W21). Левый icon-rail команд
  (Загрузить/Сохранить/Печать/Критерии + Тема/Справка). Вкладки удалены —
  критерии управляются в отдельном окне. Бренд-иконки (favicon/PWA/шапка)
  приведены к sage-палитре (W30/W32).
- **Карточка задачи**: иконки-бейджи тип/приоритет/статус с тултипами (W19/W20),
  единый бейдж Priority Score + Effort, иконка «Краткое пояснение» (SM-комментарий
  до 500 символов, модалка, печатается при заполнении, поле `note` в JSON-схеме →
  STORAGE_VERSION 13), инлайн-часы по ролям; est-поля убраны из формы.
- **Оценка критериев** — шкалы-полосы 0–10 в форме задачи (1 клик, клавиатура
  ←/→/Home/End); степперы из строк списка удалены.
- **Авто-сортировка (W27)**: список всегда DESC по Priority Score (excluded в конце)
  на: импорт/восстановление, создание, правку оценок, удаление/undo, исключение.
  Один rAF-кадр — без мерцания (RenderScheduler батчинг).
- **Печать**: диалог «Параметры печати» с чек-боксом исключённых задач (W24);
  пустые квадранты и иконка комментария не печатаются (W23/W25); sticky-заголовки
  в печати — статический поток (W38); квадранты в группах — по возрастанию №.
- **Валидация параметров спринта (W40)**: дата окончания не раньше даты начала
  (+ симметричный guard на импорте), продукт обязателен (≥3 символов после trim),
  закреплены guard'ы: дни ≥ 1, коэфф. доступности 0..100, порог алерта ≥ 0.
- **Drag живёт при re-render (W37)**: RenderScheduler.setHold держит рендер на время
  перетаскивания (класс закрыт: сброс подсветки новой задачи больше не рвёт drag).
- **No-op тогглы (W22)**: повторный клик по активной кнопке вида/плотности не
  перерисовывает список.
- **Desktop-only (W28/W31/W36, owner)**: удалены responsive.css, mobile e2e-проекты
  и бакеты, бургер/FAB-остатки, мёртвый tabController; **Recovery Center удалён
  целиком** (pre-migration backup `sprintPlannerData.backup` сохранён как
  data-safety; диагностика — только хоткей Ctrl+Alt+D).
- **A11y-фиксы из сверенных suite (W33/W34)**: контраст US-итога матрицы (dark),
  pill «Сумма весов» (light), приглушение опциональной секции формы; дубль id
  заголовка edit-модалки критериев (W21).
- **E2E reconcile (W33–W35)**: 59 красных → 0; visual-бейзлайны (9) перегенерированы
  и проверены глазами; sticky-тесты переведены на scroll-контейнер модалки.
- Версия: APP_VERSION v8.31.0, STORAGE_VERSION 13, dompurify 3.4.9 (npm + vendored).
...
| `npm run test:coverage -- --maxWorkers=50%` | 1961/1961 PASS; lines 96.47%, branches 86.63% | **0** | n/a | — |
| `npm run docs:manual-check` | 25/25 PASS | **0** | n/a | — |
| `npm run test:e2e:smoke` | 4/4 PASS, webkit workers=2 | **0** | **1** | yes |
| `npm run test:e2e` | 222/222 PASS (chromium 218, webkit 4) | **0** | **0** | no |
| `npm run release:metrics -- --smoke-summary=e2e-smoke-summary.tmp.json` | metrics JSON записан; coverage/e2e PASS, CSS 85/88 | **0** | n/a | — |
| `node scripts/report-css-important.mjs --check` | CSS important report up to date, budget 85/88 | **0** | n/a | — |
| `npm audit` | 0 vulnerabilities | **0** | n/a | — |
| `npm outdated` | no outdated packages (dompurify→3.4.9) | **0** | n/a | — |
