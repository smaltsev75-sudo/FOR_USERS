...
## Версия: март 2026 (обновление 8.11) — Исправление расчёта рабочих дней, именование файла
...
- `js/domain/config.js` — переход на `addWorkingDays` / `countWorkingDays`.
- `js/controllers/configController.js` — использование рабочих дней, автопересчёт дней при изменении даты окончания.
- `js/controllers/fileController.js` — добавлен префикс продукта в имя файла.

### Тестирование
- **Unit-тесты**: 789 → **797** (+8 тестов). Новые тесты: `isWorkingDay` (5), `addWorkingDays` (3), `countWorkingDays` (4), обновлены тесты `config.test.js`.
- **E2E-тесты**: 134 → **141** (+7 тестов), 0 провалов.

### Документация
- `UserManual.md`: уточнено описание «Дни спринта» (пропуск выходных), добавлен формат имени файла.
- `ARCHITECTURE.md`: обновлено описание `date.js`, актуализированы счётчики тестов.
