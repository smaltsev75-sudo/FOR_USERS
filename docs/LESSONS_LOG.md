...
## Ловушки v8.30.65 (Selection report UI cleanup)

- **Рекомендация не должна быть отдельной карточкой, если те же факты уже видны в сравнении.** Top-banner «Рекомендация» дублировал бейдж «Рекомендовано» и метрики карточки алгоритма. Теперь рекомендация выражена только внутри карточки: бейдж + primary apply button. Codified by: `selectionReport.test.js`, `planner.spec.js`, visual baseline `selection-report-overload`.

- **Кнопка выбора алгоритма должна жить рядом с данными, по которым пользователь выбирает.** Нижний ряд трёх apply-кнопок заставлял сопоставлять кнопку с карточкой глазами. Теперь apply-кнопка находится в своей алгоритм-карточке, а click delegation использует `closest('button')`, чтобы клик по иконке внутри кнопки тоже работал. Codified by: `selectionController.test.js`, `planner.spec.js`.

- **Справочный аккордеон не должен перебивать основной выбор.** «Об алгоритмах отбора» полезен как пояснение, но не должен стоять между сравнением и действиями. Он расположен ниже кнопки «Рекомендации», а основной путь остаётся: сравнить карточки → применить алгоритм. Codified by: `selectionReport.test.js`, `planner.spec.js`.
